...
## furthermore, each column will have a descriptive heading name
...
## This code will create a character vector that has all the Training Activity Labels in it of
##length(y_train).  
...
##R's naming convention for the headers.  I'll first have to manipulate them
...
by_subj_act <- group_by(HAR, subject, activity)
all_sum <- summarize_each(by_subj_act, funs(mean))
write.table(all_sum, file = "~/UCI HAR Dataset/tidy_sum.txt", row.names = FALSE)
